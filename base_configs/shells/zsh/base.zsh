...
for entry in \
    ...
    [[ -d "$ZSH_CUSTOM/$plugin" ]] || git clone "$url" "$ZSH_CUSTOM/$plugin"
done
...
update_fzf_theme() {
    case "$SYSTEM_THEME" in
        "echelon")
            ...
            --color=border:#404040,label:#aeaeae,query:#f8f8fb,gutter:-1
...
#=== FZF PREVIEWS ===#
# Service status
zstyle ':fzf-tab:complete:-*:*' fzf-preview 'rc-service $word status'
